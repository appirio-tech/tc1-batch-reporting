...
import com.topcoder.utilities.reporting.job.bigquery.BIgQueryJobConfig;
import com.topcoder.utilities.reporting.job.bigquery.BigQueryRunConfig;
...
public class JobRunner {
    ...
    public static void main(String[] args) throws Exception {
        ...
        BigQueryRunConfig loadConfiguration = null;
        ...
        for (BIgQueryJobConfig jc : loadConfiguration.getJobs()) {
            try {
                ...
                Dataset dataset = tableLoader.createNewDataSet(jc.getBigQueryProjectId(),
                        jc.getBigQueryDatasetId());

                // create the new table, if the table exists, it will not override
                Table table = tableLoader.createNewTable(dataset.getDatasetReference().getProjectId(),
                        dataset.getDatasetReference().getDatasetId(), jc.getBigQueryTableId(), jc.getTableSchemaFile());

                // load the job
                Job job = tableLoader.submitLoadJob(table.getTableReference(), table.getSchema(),
                        dataFileName);

                tableLoader.traceJobStatus(job);
